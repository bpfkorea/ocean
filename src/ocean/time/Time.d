...
struct TimeSpan
{
        ...
        package Duration impl;

        /// Convenience function (older compiler don't allow public alias to package)
        public Duration duration () const { return this.impl; }
        /// Allow implicit convertion from `TimeSpan` to `Duration`
        alias duration this;
        ...
        {
            return this.impl.opCmp(rhs.impl);
        }
        ...
        {
            mixin("return TimeSpan(this.impl " ~ op ~ " t.impl);");
        }

        unittest
        {
            TimeSpan time_span;
            time_span.impl = 5.hnsecs;

            TimeSpan test_span;
            test_span.impl = 10.hnsecs;

            auto res = time_span + test_span;
            test!("==")(res.ticks(), 15);

            test_span.impl = 3.hnsecs;
        ...
        {
                mixin("return TimeSpan(impl " ~ op ~ " v);");
        }
        ...
        {
            TimeSpan time_span;
            time_span.impl = 10.hnsecs;

            TimeSpan test_span;
            test_span.impl = 5.hnsecs;
        ...
        double interval()
        {
            return (cast(double) impl.total!"hnsecs") / TicksPerSecond;
        }
...
struct Time
{
        ...
        **********************************************************************/
        ...
        {
            mixin("return Time(ticks_ " ~ op ~ " t.impl.total!`hnsecs`);");
        }
        ...
        unittest
        {
            ...
            TimeSpan test_span;
            test_span.impl = 10.dur!"hnsecs";

            auto res = time_span + test_span;
            test!("==")(res.ticks(), 15);

            test_span.impl = 3.dur!"hnsecs";
        ...
        **********************************************************************/
        ...
        {
                mixin("ticks_ " ~ op ~ "= t.impl.total!`hnsecs`;");
                return this;
        }
...
struct TimeOfDay
{
        ...
        {
            // Note: Since this represent a time of day, hours needs to be
            // 0 <= hours < 24. Usinf `days` ensures that, even if we don't
            // use the value.
